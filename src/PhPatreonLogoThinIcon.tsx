import { JSX, splitProps, mergeProps } from "solid-js";

export interface PhPatreonLogoThinIconProps extends JSX.SvgSVGAttributes<SVGSVGElement> {
  size?: number | string;
  color?: string;
}

/**
 * ![img](data:image/svg+xml;base64,)
 */
export function PhPatreonLogoThinIcon(props: PhPatreonLogoThinIconProps) {
  const merged = mergeProps({ size: "1em", color: "currentColor" }, props);
  const [local, svgProps] = splitProps(merged, ["size", "color"]);
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 256 256"
      fill={local.color}
      width={local.size}
      height={local.size}
      {...svgProps}
    >
      <path d="M185.85,35.7c-18.11-7.41-39.68-9.56-60.76-6.06C102.5,33.4,83,43.1,68.62,57.7,58,68.5,45.11,89.65,44.06,129c-.69,25.95,4.29,54.29,13.3,75.79C65.8,224.92,76.51,236,87.52,236c20.5,0,29.78-18.13,39.61-37.32,7.19-14.05,14.63-28.57,26.86-36.87,6.25-4.23,14.39-7.37,23-10.7,22.72-8.76,51-19.67,51-57.94C228,67.6,212.64,46.66,185.85,35.7ZM174.13,143.64c-9.06,3.5-17.63,6.8-24.63,11.55-13.93,9.45-22.2,25.6-29.49,39.84-9.06,17.69-16.88,33-32.49,33-7.36,0-15.88-9.83-22.78-26.3-8.6-20.51-13.34-47.61-12.68-72.49,1-36.75,12.64-56.12,22.26-65.9,18.86-19.19,45.53-27.23,70.33-27.23,13.88,0,27.18,2.52,38.17,7C215.17,56.34,220,80.15,220,93.17,220,126,196.68,135,174.13,143.64Z"/>
    </svg>
  );
}
