import { createIcon } from "./createIcon.js";

export type { PhIconProps as PhMapPinSimpleFillIconProps } from "./createIcon.js";

/**
 * ![img](data:image/svg+xml;base64,)
 */
export const PhMapPinSimpleFillIcon = createIcon(`<path d="M136,127.42V232a8,8,0,0,1-16,0V127.42a56,56,0,1,1,16,0Z"/>`);
